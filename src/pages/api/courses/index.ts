...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCourse() {
    ...
    const body = { ...req.body };
    if (body?.learning_path?.length > 0) {
      const create_learning_path = body.learning_path;
      body.learning_path = {
        create: create_learning_path,
      };
    } else {
      delete body.learning_path;
    }
    ...
    if (body?.teacher_course?.length > 0) {
      const create_teacher_course = body.teacher_course;
      body.teacher_course = {
        create: create_teacher_course,
      };
    } else {
      delete body.teacher_course;
    }
